...
srqflights %>% 
  group_by(month, flight, origin, carrier) %>% 
  summarise(avg_delay = mean(dep_delay, na.rm = TRUE),
            median_delay = mean(dep_delay, na.rm = TRUE),
            flight_count = n())
...
```
## how many delta flights were delayed in the month of august on averagw?
...
## how many delta flights were delayed flying into DCA in the month of august on average?

```{r}
test <- flights %>% 
  filter(carrier == "DL", dest == "DCA") %>% 
  group_by(month) %>% 
  summarise(avg_delay = mean(dep_delay, na.rm = TRUE), 
            flight_count = n())
...
srqflights %>% 
  ...
  mutate(pct = flight_count / sum(flight_count))
